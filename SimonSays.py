...
class singlePlayer(Frame):
    ...
    def selectedTile(self, color):
        ...
        #plays sound
       # pygame.mixer.Sound(random.choice(listofsoundspaths)).play()
        #subprocess.call(["afplay", random.choice(listofsoundspaths)])
        #os.system(random.choice(listofsoundspaths))
        #playsound(random.choice(listofsoundspaths))

        if(platform == "linux" or platform == "linux2" or platform == "darwin"):
            wave_obj = sa.WaveObject.from_wave_file(random.choice(listofsoundspaths))
